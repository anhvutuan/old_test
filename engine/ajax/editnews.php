...
if( $_REQUEST['action'] == "edit" ) {
	...
	if ($config['allow_multi_category']) {
		...
		$cats = "<select data-placeholder=\"{$lang['addnews_cat_sel']}\" name=\"catlist[]\" id=\"edit_category_list\" onchange=\"onCategoryChange(this)\" style=\"width:350px;height:140px;\" multiple=\"multiple\">";
	} else {

		$cats = "<select data-placeholder=\"{$lang['addnews_cat_sel']}\" name=\"catlist[]\" id=\"edit_category_list\" onchange=\"onCategoryChange(this)\" style=\"width:350px;\">";
	}
	...
	foreach ($xfields as $name => $value) {
		...
		if ($value[3] == "textarea") {
			...
			$xfbuffer .= "<div id=\"{$holderid}\" class=\"xfieldsrow\"><div class=\"xfieldscolleft\">{$value[1]}:</div><div class=\"xfieldscolright\"><input type=\"checkbox\" name=\"xfield[{$fieldname}]\" value=\"1\"" . ($fieldvalue ? "checked " : "") . "></div></div>";
...
HTML;
	...
	$db->query( "UPDATE " . PREFIX . "_post SET title='{$_POST['title']}', short_story='{$news_txt}', full_story='{$full_txt}', xfields='{$filecontents}', category='{$category_list}', approve='{$approve}', allow_br='{$allow_br}' WHERE id = '{$id}'" );
