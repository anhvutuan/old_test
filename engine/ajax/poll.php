...
HTML;
	...
	if( ! $poll['multiple'] ) {
		...
		for($v = 0; $v < sizeof( $body ); $v ++) {
			...
			$buffer .= <<<HTML
<div class="pollanswer"><label for="vote{$news_id}{$v}"><input id="vote{$news_id}{$v}" name="dle_poll_votes" type="radio" value="{$v}"><span>{$body[$v]}</span></label></div>
HTML;
		...
		for($v = 0; $v < sizeof( $body ); $v ++) {
			...
			$buffer .= <<<HTML
<div class="pollanswer"><label for="vote{$news_id}{$v}"><input id="vote{$news_id}{$v}" name="dle_poll_votes[]" type="checkbox" value="{$v}"><span>{$body[$v]}</span></label></div>
HTML;
