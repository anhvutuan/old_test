...
class DLE_Comments {
	...
	function compile_comment( $tpl, $row, $area, $indent ) {
		...
		if ($area == 'lastcomments') {
			...
			$row['title'] = html_entity_decode($row['title'], ENT_QUOTES, $config['charset']);
		...
		if( ($member_id['name'] == $row['name'] AND $row['is_register']) OR !$user_group[$member_id['user_group']]['allow_complaint_comments'] ) {
