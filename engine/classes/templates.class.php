...
class dle_template {
	...
	function compile_global_tags( $content ) {
		...
		if (stripos($content, "[active-plugins=") !== false) {
			$content = preg_replace_callback("#\\[(active-plugins)=(.+?)\\](.*?)\\[/active-plugins\\]#is", array(&$this, 'check_plugins'), $content);
		}

		if (stripos($content, "[not-active-plugins=") !== false) {
			$content = preg_replace_callback("#\\[(not-active-plugins)=(.+?)\\](.*?)\\[/not-active-plugins\\]#is", array(&$this, 'check_plugins'), $content);
		}
