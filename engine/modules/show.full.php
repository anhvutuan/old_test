...
if( !defined('DATALIFEENGINE') ) {
	...
	if ( is_array($row) ) {
		...
		$result = $db->execute_query($query, [$newsid]);
	...
	if ( isset($row['id']) AND $row['id'] ) {
		...
		if ($row['id'] AND $need_pass AND $member_id['user_group'] > 2 ) {
			...
			if( trim($_POST['news_password']) ) {
				$query = "SELECT password FROM " . PREFIX . "_post_pass WHERE news_id = ?";
	...
	if ( isset($row['id']) AND $row['id'] AND  $perm ) {
		...
		if( stripos( $tpl->copy_template, "{next-" ) !== false OR stripos( $tpl->copy_template, "{prev-" ) !== false) {
			...
			if( !is_array($prev_next) ) {
				...
				$query = "SELECT id, date, title, category, alt_name FROM " . PREFIX . "_post WHERE category = ? AND date >= ? AND id != ? AND approve = 1 ORDER BY date ASC LIMIT 1";
				$result = $db->execute_query($query, [$row['category'], $row['date'], $row['id']]);
				$row_link = $result->fetch_assoc();
				...
				$query = "SELECT id, date, title, category, alt_name FROM " . PREFIX . "_post WHERE category = ? AND date <= ? AND id != ? AND approve = 1 ORDER BY date DESC LIMIT 1";
				$result = $db->execute_query($query, [$row['category'], $row['date'], $row['id']]);
				$row_link = $result->fetch_assoc();
...
HTML;
		...
		if (strpos($tpl->copy_template, "[updated]") !== false or strpos($tpl->copy_template, "[not-updated]") !== false) {
			...
			if ($config['post_updated'] AND $row['editdate'] AND compare_days_date($row['date'],  $short_news_cache, true) > $config['post_new'] AND compare_days_date($row['editdate'],  $short_news_cache, true) < $config['post_updated'] ) {
				$tpl->set('[updated]', "");
				$tpl->set('[/updated]', "");
...
HTML;
		...
		if (!$is_logged) {
			...
			$cookie_guest_name = isset($_COOKIE['dle_guest_name']) ? htmlspecialchars( strip_tags( stripslashes($_COOKIE['dle_guest_name']) ), ENT_QUOTES, $config['charset'] ) : '';
			$cookie_guest_mail = isset($_COOKIE['dle_guest_mail']) ? htmlspecialchars( strip_tags( stripslashes($_COOKIE['dle_guest_mail']) ), ENT_QUOTES, $config['charset']) : '';
