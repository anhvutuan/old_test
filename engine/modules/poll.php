...
if( !$log['count'] AND $user_group[$member_id['user_group']]['allow_poll'] AND !$poll['closed'] ) {
	if( ! $poll['multiple'] ) {
		...
		for($v = 0; $v < sizeof( $body ); $v ++) {
			...
			$list .= <<<HTML
<div class="pollanswer"><label for="vote{$row['id']}{$v}"><input id="vote{$row['id']}{$v}" name="dle_poll_votes" type="radio" value="{$v}"><span>{$body[$v]}</span></label></div>
HTML;
		...
		for($v = 0; $v < sizeof( $body ); $v ++) {
			...
			$list .= <<<HTML
<div class="pollanswer"><label for="vote{$row['id']}{$v}"><input id="vote{$row['id']}{$v}" name="dle_poll_votes[]" type="checkbox" value="{$v}"><span>{$body[$v]}</span></label></div>
HTML;
